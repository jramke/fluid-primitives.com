...
|------|------|-------------|----------|--------|
| `name` | string | Name of component to use the props from | No | - |
| `defaults` | array | Default values for props to override the imported ones. Key-value pairs | Yes | [] |
| `props` | array | Only use a subset of props from the referenced component. Value should be an array of prop names. | Yes | [] |
